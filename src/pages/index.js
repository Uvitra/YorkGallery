...
export default class IndexPage extends React.Component {
	...
	componentDidMount() {
		const { edges: entry } = this.props.data.allMarkdownRemark
	...
	handleTagFilter(category) {
		let filteredArray = []
		category === 'reset'
			? (filteredArray = this.state.queryArray)
				...
				<nav className={GalleryStyle.navbar}>
					<div className={GalleryStyle.navbarContainer}>
						<svg
							...
							onClick={() => this.handleTagFilter('reset')}
						>
							<path
								id="path2588"
				...
				<section className={GalleryStyle.mainContainer}>
					...
					<article className={GalleryStyle.studentNamesFlexContainer}>
						<ul
							...
							style={
								this.state.filteredQueryArray.length > 15
									? { width: '100vw', columnCount: '4' }
									: this.state.filteredQueryArray.length > 10
										? { width: '75vw', columnCount: '3' }
										: this.state.filteredQueryArray.length > 5
											? { width: '50vw', columnCount: '2' }
											: this.state.filteredQueryArray.length > 0
												? { columnCount: '1' }
												: {}
							}
						>
					...
					>
						...
						<div className={GalleryStyle.projectMediaContainer}>
							{this.state.projectEdgeSelected !== -1
								? this.props.data.allMarkdownRemark.edges[
									this.state.projectEdgeSelected
								  ].node.frontmatter.projectImagesList.map(
									(imageAndAlt, index) => (
										<img
											key={index}
